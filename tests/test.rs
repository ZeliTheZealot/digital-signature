...
//    //byte literal array
//    let seckey_a: [u8; 32] = [
//        169, 195, 92, 103, 2, 159, 75, 46,
//        158, 79, 249, 49, 208, 28, 48, 210,
//        5, 47, 136, 77, 21, 51, 224, 54,
//        213, 165, 90, 122, 233, 199, 0, 248];
//    //parse from array to SecretKey struct
//    let seckey = SecretKey::parse(&seckey_a).unwrap();
//    let pubkey = PublicKey::from_secret_key(&seckey);
//    //initialise message byte literal array and parse to Message struct
//    let message_arr = [6u8; 32];
//    let message = Message::parse(&message_arr);
//
//    //signing
//    let (sig, recid) = sign(&message, &seckey); //note that they forgot to do match, so this is bug
//    //my attempt at fixing the bug failed
//    //recall recovery id is a u8
//    let tmp: u8 = recid.into();
//    //the following is probably a test case (they've known a priori that this should == 1)
//    assert_eq!(tmp, 1u8);
//
//    //recovering into PublicKey type
//    let recovered_pubkey = recover(&message, &sig, &recid).unwrap();
//    //getting a full public key (recovered public key array)
//    let rpa = recovered_pubkey.serialize();
//    //this is from earlier (original public key array)
//    let opa = pubkey.serialize();
//    let rpr: &[u8] = &rpa;
//    let opr: &[u8] = &opa;
//    assert_eq!(rpr, opr);
